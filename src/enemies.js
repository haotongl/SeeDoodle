...
export class EnemyManager {
  ...
  spawn(type, pos, id = null) {
    ...
    const e = { type, T, mat, root: model.root, parts: model.parts, J: model.J, tip: model.tip, face: model.face, hit: model.hit, hp: T.hp, maxHp: T.hp, alive: true, state: 'spawn', t: 0,
      ...
      slideT: 0, slideS: Math.random() < 0.5 ? 1 : -1, progD: Infinity, progT: 0, progI: -2, hopT: 0,
  ...
  _explodeBomber(e, scale = 1) {
    ...
    // on a mirror this is scenery: the host already worked out who the blast caught
    if (!this.mirror) {
  ...
  _steer(e, dt, gx, gz, speed, accel) {
    ...
    dx /= l; dz /= l;
    if (e.slideT > 0) e.slideT -= dt;
    // Slide along whatever we are pressed against. Physics zeroes the velocity on the blocked
    // axis every frame, so an enemy steering straight into a wall spends its whole acceleration
    // budget on the wall and never gets round the corner.
    const wn = b.wallNormal;
    if (!e.T.flying && b.hitWall && (wn.x || wn.z)) {
      const into = -(dx * wn.x + dz * wn.z);
      if (into > 0.2) {
        let tx = -wn.z, tz = wn.x; const along = tx * dx + tz * dz;
        // Head-on there is no tangent to prefer, and picking afresh each frame leaves them
        // shivering against the wall, so commit to one side for a while.
        if (Math.abs(along) < 0.25) {
          if (e.slideT <= 0) { const vt = tx * b.vel.x + tz * b.vel.z; e.slideS = Math.abs(vt) > 0.5 ? Math.sign(vt) : (Math.random() < 0.5 ? 1 : -1); e.slideT = 1.3; }
          if (e.slideS < 0) { tx = -tx; tz = -tz; }
        }
        else if (along < 0) { tx = -tx; tz = -tz; }
        const k = Math.min(0.95, into * 1.5);
        dx += (tx - dx) * k; dz += (tz - dz) * k;
        const nl = Math.hypot(dx, dz) || 1; dx /= nl; dz /= nl;
      }
    }
    const a = (b.onGround ? accel : accel * 0.3) * dt; speed *= this.mods.speed;
  ...
  _follow(e, dt, target, speed) {
    ...
    // A path we have walked off the end of, or one that never reached the goal, is worth nothing.
    // Sitting on it until the timer runs out is how an enemy ends up shoving a wall for a second.
    const spent = !e.path || e.pathI >= e.path.length;
    const stale = spent || !e.path.complete || !e.pathGoal || e.pathGoal.distanceTo(target) > 3.5;
    if (stale && (e.pathT <= 0 || spent)) {
      ...
      else if (spent) { e.path = null; e.pathGoal = null; }
    }
    let goal = null;
    if (e.path && e.pathI < e.path.length) {
      // Skip ahead past any waypoint we can already walk straight past: smoothing leaves the
      // odd corner behind us and turning back for it is exactly the kind of shuffle that jams.
      while (e.pathI < e.path.length - 1 && e.path[e.pathI + 1].distanceToSquared(b.pos) < 30 && nav.walkClear(b.pos, e.path[e.pathI + 1])) e.pathI++;
      const nd = e.path[e.pathI]; const hd = Math.hypot(nd.x - b.pos.x, nd.z - b.pos.z);
      if (hd < 0.5 && Math.abs(nd.y - b.pos.y) < 1.2) { e.pathI++; if (e.pathI < e.path.length) goal = e.path[e.pathI]; } else goal = nd;
    }
    if (!goal) goal = target;
    this._steer(e, dt, goal.x, goal.z, speed, 40);
    const hd = Math.hypot(goal.x - b.pos.x, goal.z - b.pos.z);
    if (e.hopT > 0) e.hopT -= dt;
    if (!b.onGround) return;   // airborne time is not evidence either way, but do not throw away what we had
    // Watchdog on progress, not on contact. Pressing a wall is only one way to be stuck: circling
    // a pillar, swapping places with a neighbour and bouncing between two waypoints all look like
    // movement and get nowhere, so measure the one thing that matters — closing on the waypoint.
    const gi = e.path && e.pathI < e.path.length ? e.pathI : -1;
    if (gi !== e.progI) { e.progI = gi; e.progD = hd; e.progT = 0; }
    else if (hd < e.progD - 0.2) { e.progD = hd; e.progT = 0; }
    else e.progT += dt;
    // decay rather than reset, so a corner that lets go for one frame in three still counts
    if (b.hitWall) e.stuckT += dt; else e.stuckT = Math.max(0, e.stuckT - dt * 1.5);
    const jam = Math.max(e.stuckT, e.progT * 0.9);
    if (goal.y > b.pos.y + 0.6 && hd < 1.7) { b.vel.y = 9; b.onGround = false; e.stuckT = 0; e.progT = 0; }
    else if (jam > 2.4) {
      // Nothing worked. Give up on this waypoint entirely, take a fresh route, and pick a
      // different slot around the target so we stop being funnelled into the same gap.
      e.path = null; e.pathGoal = null; e.pathT = 0; e.stuckT = 0; e.progT = 0; e.slideT = 0; e.hopT = 0;
      e.appT = 0; e.appAng += rand(1.7, 2.5) * (Math.random() < 0.5 ? -1 : 1);
    } else if (jam > 1.3 && e.hopT <= 0) {
      // Hop a kerb, a crate, or a deadlocked neighbour. On a cooldown, and the progress clock is
      // left running: if hopping is not working the ladder has to keep climbing to the rung that
      // abandons the route, instead of bouncing on this one for the rest of the round.
      b.vel.y = 9; b.onGround = false; e.hopT = 0.7; e.stuckT = 0;
    } else if (jam > 0.5 && e.slideT <= 0) {
      e.slideS = Math.random() < 0.5 ? 1 : -1; e.slideT = 1.4;   // commit to one way round before climbing
      e.pathGoal = null; e.pathT = 0;
    }
  }
